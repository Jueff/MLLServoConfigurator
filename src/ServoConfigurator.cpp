 ...
 *
 * ServoConfigurator implements a state machine for configuring up to three servo channels.
 * It translates LED PWM signals into control commands, allows setting min/max limits,
 * speed, and positions, and stores them persistently in flash memory.
 * The class uses StatefulServoController instances for individual servo management
 * and expects regular calls to process() for updates.
 * 
 * ServoConfigurator realisiert eine Zustandsmaschine zur Konfiguration von bis zu drei Servo-Kanälen.
 * Sie übersetzt LED-PWM-Signale in Steuerbefehle, ermöglicht die Einstellung von Min/Max-Grenzen,
 * Geschwindigkeit und Positionen, und speichert diese persistent im Flash-Speicher.
 * Die Klasse nutzt StatefulServoController-Instanzen für die individuelle Servo-Verwaltung
 * und erwartet regelmäßige Aufrufe von process() zur Aktualisierung.
 *
